...
* @param {number}           [cost=1] quantity of Marbles to initially create    
...
function createMarble(color, quantity=1, cost=1, callback) {
  cmd.get(
    'bright console issue command "F CICSTRN1,MB01 CRE ' + color + " " + quantity + " " + cost + '" --cn CUST001',
    function (err, data, stderr) {
...
function deleteMarble(color, callback) {
  cmd.get(
    'bright console issue command "F CICSTRN1,MB01 DEL ' + color + '" --cn CUST001',
    function (err, data, stderr) {
      typeof callback === 'function' && callback(err, data, stderr);
    }
  );
...
function getMarbleQuantity(color, callback) {
  ...
  cmd.get(
    'bright jobs submit data-set "CUST001.MARBLES.JCL(MARBDB2)" --rff jobid --rft string',
    function (err, data, stderr) {
      if(err){
        callback(err);
      } else {
...
function updateMarble(color, quantity, callback) {
  cmd.get(
    'bright console issue command "F CICSTRN1,MB01 UPD ' + color + " " + quantity + '" --cn CUST001',
    function (err, data, stderr) {
      typeof callback === 'function' && callback(err, data, stderr);
    }
  );
